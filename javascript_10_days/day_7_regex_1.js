var input = ` <div class="question-summary" id="question-summary-80407">
...
function processData(input) {
  ...
  let productName = [];
  let productTime = [];
  let re = new RegExp(/href="\/questions\/(\d+)\/[\w-]*"\s*class="question-hyperlink">(.*)<\/a>/g);
  while (match = re.exec(input)) {
      productId.push(match[1]);
      productName.push(match[2]);
  }

  re = new RegExp(/<div\sclass="user-action-time\s*"\s*>\n*.*<span.*relativetime.*">(.*)<\/span>/g);
  while (match = re.exec(input)) {
      productTime.push(match[1]);
  }    
  for(let i=0; i < productId.length; i ++) {
      console.log(productId[i] + ';' + productName[i] + ';' + productTime[i])
  }
